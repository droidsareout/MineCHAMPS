...
world.afterEvents.worldLoad.subscribe(() =>
    system.runInterval(() => {
        world
            ...
            .getEntities({ type: 'minechamps:floating_text' })
            .forEach((entity) => {
                const tags = entity.getTags();

                let scoreboard = tags.find((tag) =>
                    tag.startsWith('scoreboard:'),
                );
                ...
                scoreboard.getScores().forEach((scoreInfo) => {
                    const name =
                        scoreInfo.participant
                            .getEntity()
                            .getTags()
                            .filter((tag) => tag.startsWith('name:'))
                            .map((tag) => tag.slice(5))[0] ??
                        scoreInfo.participant.displayName;
                    const score = scoreInfo.score;

                    if (
                        !showOffline &&
                        name == 'commands.scoreboard.players.offlinePlayerName'
                    )
                        return;
                ...
                for (let i = 1; i <= Math.min(10, participants.length); i++) {
                    ...
                    text.push(
                        `§l${number}§r§8...§7${participant.name} §c${participant.score}`,
                    );
                }
