...
system.beforeEvents.startup.subscribe((event) => {
        ...
        (origin) => {
            if (origin.sourceEntity.hasTag('inGame')) {
                ...
                origin.sourceEntity.sendMessage(
                    '§vYou cannot return to spawn while in game!',
                );
                ...
                system.runTimeout(
                    () => origin.sourceEntity.playSound('mob.endermen.portal'),
                    1,
                );
    ...
    event.customCommandRegistry.registerCommand(
        ...
        (origin) => {
            system.run(() =>
                origin.sourceEntity.sendMessage(
                    '§gDiscord server code: §UyJtnkyUfE',
                ),
            );
    ...
    event.customCommandRegistry.registerCommand(
        ...
        (origin) => {
            system.run(() => {
                ...
                world.sendMessage(
                    `§uChat cleared by ${origin.sourceEntity.name}.`,
                );
                world
                    .getPlayers()
                    .forEach((players) => players.playSound('random.pop'));
            });
        },
    );
    ...
    event.customCommandRegistry.registerCommand(
        ...
        (origin) => {
            system.run(() => {
                origin.sourceEntity.teleport(
                    { x: 0.5, y: -60, z: -249.5 },
                    { facingLocation: { x: 0.5, y: -60, z: -250.5 } },
                );
                system.runTimeout(
                    () => origin.sourceEntity.playSound('mob.endermen.portal'),
                    1,
