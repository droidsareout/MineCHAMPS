...
system.runInterval(() => {
    world.getPlayers().forEach((player) => {
        ...
        for (let i = 0; i < scoreboards.length; i++) {
            if (
                scoreboard.getObjective(scoreboards[i]).getScore(player) ===
                undefined
            ) {
...
system.runInterval(() => {
    world.getPlayers().forEach((player) => {
        ...
        let inGameCount = Number(
            JSON.stringify(
                player.dimension.getPlayers({ tags: ['inGame'] }).length,
            ),
        );
        let spectatingGameCount = Number(
            JSON.stringify(
                player.dimension.getPlayers({ tags: ['spectatingGame'] })
                    .length,
            ),
        );
        let optedInCount = Number(
            JSON.stringify(
                player.dimension.getPlayers({ tags: ['optedIn'] }).length,
            ),
        );
