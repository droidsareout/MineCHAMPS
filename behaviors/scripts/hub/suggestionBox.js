...
world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
    ...
    if (event.block.typeId === 'minecraft:jukebox') {
        ...
        const mainHandSlot = inventory.container.getItem(
        ...
        system.run(() => {
            ...
            for (let i = 0; i < inventory.container.size; i++) {
                if (
                    inventory.container.getItem(i)?.typeId ===
                        'minecraft:writable_book' &&
                    inventory.container.getItem(i)?.nameTag ===
                        '§r§nSuggestion Book'
                ) {
                    player.sendMessage(
                        '§vYou can only grab one §nSuggestion Book§v at a time.',
                    );
